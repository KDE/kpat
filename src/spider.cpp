...
#include "patsolve/spidersolver2.h"
#include "pileutils.h"
#include "settings.h"
#include "speeds.h"
// KF
...
bool Spider::checkAdd(const PatPile *pile, const QList<KCard *> &oldCards, const QList<KCard *> &newCards) const
{
    ...
    return pile->pileRole() == PatPile::Tableau && (oldCards.isEmpty() || oldCards.last()->rank() == newCards.first()->rank() + 1);
}

bool Spider::checkRemove(const PatPile *pile, const QList<KCard *> &cards) const
{
    return pile->pileRole() == PatPile::Tableau && isSameSuitDescending(cards);
}
...
void Spider::cardsMoved(const QList<KCard *> &cards, KCardPile *oldPile, KCardPile *newPile)
{
    ...
    if (!isDemoActive() && p && p->pileRole() == PatPile::Tableau && pileHasFullRun(p)) {
        m_pilesWithRuns << p;
    }
...
bool Spider::pileHasFullRun(KCardPile *pile)
{
    ...
    return pile->count() >= 13 && potentialRun.first()->isFaceUp() && isSameSuitDescending(potentialRun);
}
