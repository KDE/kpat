...
int SpiderSolver::get_possible_moves(int *a, int *numout)
{
    ...
    for (int w = 0; w < 10; ++w) {
        if (Wlen[w] >= 13 && RANK(*Wp[w]) == PS_ACE) {
            ...
            bool stroke = true;
            for (int l = 0; l < 13; ++l) {
                if (DOWN(W[w][Wlen[w] - l - 1]) || RANK(W[w][Wlen[w] - l - 1]) != l + 1 || SUIT(W[w][Wlen[w] - l - 1]) != ace_suit) {
                    stroke = false;
                    break;
                }
            }
    ...
    *numout = n;
    ...
    for (int j = 0; j < 10; ++j) {
        ...
        for (; conti[j] < Wlen[j] - 1; ++conti[j]) {
            if (SUIT(*Wp[j]) != SUIT(W[j][Wlen[j] - conti[j] - 2]) || DOWN(W[j][Wlen[j] - conti[j] - 2]))
                break;
            if (RANK(W[j][Wlen[j] - conti[j] - 1]) != RANK(W[j][Wlen[j] - conti[j] - 2]) - 1)
                break;
        }
        conti[j]++;
    }
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        for (int l = 0; l < len; ++l) {
            ...
            for (int j = 0; j < 10; ++j) {
                ...
                if (Wlen[j] > 0 && RANK(card) == RANK(*Wp[j]) - 1) {
                    allowed = true;
#if 1
                ...
                if (allowed && Wlen[i] >= l + 2 && Wlen[i] > 1) {
                    ...
                    if (SUIT(card) == SUIT(card_below) && !DOWN(card_below) && RANK(card_below) == RANK(card) + 1) {
#if 0
                    ...
                    if (Wlen[j]) {
                        if (SUIT(card) != SUIT(*Wp[j]))
                            ...
                            if (conti[j] + l + 1 != 13 || conti[i] > conti[j] + l) {
                                ...
                                if (!exists_card_below || ((SUIT(card_below) != SUIT(card) || RANK(card_below) != RANK(card) + 1))) {
                                    foundgood = true;
                                } else {
                                    toomuch++;
                                    mp->pri = -40;
